...
const MotolinkLanding = ({ isAuthenticated, onLoginRequired, onBook, onLogout, onViewShop }: MotolinkLandingProps) => {
  ...
  return <div className="min-h-screen bg-white text-slate-900">
    ...
    <main>
      ...
      <motion.section initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.45 }} id="shops" className="scroll-mt-20 bg-slate-50 px-4 py-16 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><div className="mb-8 flex flex-wrap items-end justify-between gap-5"><div><p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-400">Partner network</p><h2 className="mt-3 font-display text-4xl uppercase leading-none tracking-wide text-slate-900 sm:text-5xl">Explore Partner Shops<span className="text-[#E23428]">.</span></h2><p className="mt-3 text-sm text-slate-600">{results.length} live partner shop{results.length === 1 ? "" : "s"} in the network.</p></div><a href="#map" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-700 underline decoration-slate-300 underline-offset-4 transition hover:text-[#E23428] hover:decoration-[#E23428] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E23428] focus-visible:ring-offset-4">View all shops <ArrowUpRight size={16} /></a></div>{results.length ? <ShopGallery shops={results} onSelect={setSelectedShop} onConnect={connect} onViewShop={onViewShop} /> : <div className="rounded-2xl border border-dashed border-slate-300 p-10 text-center text-slate-600"><Search className="mx-auto mb-3" />No shops match these filters. Try a broader search.</div>}</div></motion.section>
