...
const ShopFilters = ({ specialties, specialty, availabilityOnly, city, onSpecialtyChange, onAvailabilityChange, onCityChange }: ShopFiltersProps) => {
  ...
  const hasActiveFilters = Boolean(specialty || availabilityOnly || city);

  return (
    <div className="rounded-2xl border border-slate-200/60 bg-white/95 shadow-xl backdrop-blur-md">
      {/* Toggle bar */}
      <button
        onClick={() => setExpanded((v) => !v)}
        className="flex w-full items-center justify-between gap-2 rounded-2xl px-4 py-3 text-left transition hover:bg-slate-50 sm:cursor-pointer"
      >
        <span className="flex items-center gap-2 text-sm font-semibold text-slate-800">
          ...
          {hasActiveFilters && (
            <span className="inline-flex h-5 min-w-[20px] items-center justify-center rounded-full bg-teal-700 px-1.5 text-[10px] font-bold text-white">
              {(specialty ? 1 : 0) + (availabilityOnly ? 1 : 0) + (city ? 1 : 0)}
            </span>
          )}
        </span>
        ...
        <div className="space-y-3 border-t border-slate-100 px-4 pb-4 pt-3">
          <label className="relative block">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-3 text-sm text-slate-800 placeholder:text-slate-400 outline-none transition focus:border-teal-600 focus:ring-2 focus:ring-teal-600/20"
            />
          </label>
