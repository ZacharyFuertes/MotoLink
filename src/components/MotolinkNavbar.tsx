...
const MotolinkNavbar = ({ isAuthenticated, onBrowse, onMap, onAbout, onLogin, onSignup, onLogout }: MotolinkNavbarProps) => {
  ...
  return <header className="fixed inset-x-0 top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
    <div className="mx-auto flex h-24 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
      ...
      <nav className="hidden items-center gap-7 md:flex">{links.map((link) => <button key={link.label} onClick={link.action} className="text-sm font-medium text-slate-600 hover:text-slate-950 whitespace-nowrap">{link.label}</button>)}</nav>
      <div className="hidden items-center gap-3 md:flex">{isAuthenticated ? <>
              <button onClick={onBrowse} className="rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-700 whitespace-nowrap">Browse shops</button>
              {onLogout && <button onClick={onLogout} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-100 whitespace-nowrap"><LogOut size={16} /> Log out</button>}
            </> : <><button onClick={onSignup} className="rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-100 whitespace-nowrap">Register</button><button onClick={onLogin} className="rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-700 whitespace-nowrap">Log in</button></>}</div>
    ...
    {menuOpen && <div className="border-t border-slate-200 bg-white px-4 py-4 md:hidden"><div className="mx-auto flex max-w-7xl flex-col gap-2">{links.map((link) => <button key={link.label} onClick={() => activate(link.action)} className="rounded-lg px-3 py-2 text-left text-sm font-medium text-slate-700 hover:bg-slate-100">{link.label}</button>)}{isAuthenticated ? <>
