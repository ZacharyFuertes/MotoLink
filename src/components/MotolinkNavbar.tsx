  ...
  return <header className="fixed inset-x-0 top-0 z-40 border-b border-moto-gray bg-moto-darker/95 backdrop-blur-sm">
    <div className="mx-auto flex h-24 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
      ...
      <div className="hidden items-center gap-3 md:flex">{isAuthenticated ? <>
            ...
            </> : <><button onClick={onSignup} className="rounded-xl bg-moto-accent px-4 py-2.5 text-sm font-bold text-slate-950 hover:bg-moto-accent-dark whitespace-nowrap">Register</button><button onClick={onLogin} className="rounded-xl border border-moto-accent px-4 py-2.5 text-sm font-bold text-moto-accent hover:bg-moto-accent hover:text-slate-950 whitespace-nowrap">Log in</button></>}</div>
    ...
    </> : <div className="mt-2 flex flex-col gap-2"><button onClick={() => activate(onSignup)} className="rounded-lg bg-moto-accent px-3 py-2.5 text-sm font-bold text-slate-950">Register</button><button onClick={() => activate(onLogin)} className="rounded-lg border border-moto-accent px-3 py-2.5 text-sm font-bold text-moto-accent">Log in</button></div>}</div></div>}
  </header>;
};
