...
const MotolinkNavbar = ({ isAuthenticated, onBrowse, onMap, onAbout, onLogin, onSignup }: MotolinkNavbarProps) => {
  ...
  return <header className="fixed inset-x-0 top-0 z-40 border-b border-slate-200/80 bg-white/95 backdrop-blur">
    <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
      <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="flex items-center gap-3 text-left"><img src={motolinkLogo} alt="Motolink Autoshop Clientele" className="h-10 w-10 rounded-lg object-contain" /><span className="font-bold tracking-tight text-slate-900">Motolink <span className="font-medium text-slate-500">Autoshop Clientele</span></span></button>
    ...
    {menuOpen && <div className="border-t border-slate-200 bg-white px-4 py-4 md:hidden"><div className="mx-auto flex max-w-7xl flex-col gap-2">{links.map((link) => <button key={link.label} onClick={() => activate(link.action)} className="rounded-lg px-3 py-2 text-left text-sm font-medium text-slate-700 hover:bg-slate-100">{link.label}</button>)}<button onClick={() => activate(isAuthenticated ? onBrowse : onLogin)} className="mt-2 rounded-lg bg-slate-900 px-3 py-2.5 text-sm font-semibold text-white">{isAuthenticated ? "Browse shops" : "Log in or register"}</button></div></div>}
  </header>;
};
